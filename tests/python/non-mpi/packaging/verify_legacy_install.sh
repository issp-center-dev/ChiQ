...
while IFS= read -r name; do
  case "$name" in
    CHIQ_LEGACY_VERIFIER_INTERNAL|PATH|TMPDIR|PYTHON_REQUEST|LANG|LC_ALL|LC_CTYPE|CC|CXX|SDKROOT|EIGEN3_INCLUDE_DIR|PYTHON_INCLUDE_DIR|PYTHON_LIBRARY|JOBS|PWD|SHLVL|_)
      ;;
...
POLICY_ROOT=$(mktemp -d "$TMPDIR/chiq-legacy-policy.XXXXXX")
...
print_environment_policy() {
  ...
  echo "Restored names: LANG LC_ALL LC_CTYPE PATH TMPDIR PYTHON CC CXX SDKROOT EIGEN3_INCLUDE_DIR PYTHON_INCLUDE_DIR PYTHON_LIBRARY JOBS"
  echo "Controlled names: HOME(private) GIT_CONFIG_GLOBAL GIT_CONFIG_NOSYSTEM PYTEST_DISABLE_PLUGIN_AUTOLOAD=1"
  echo "Scrubbed names: PYTHON* PYTEST_* VIRTUAL_ENV CONDA_PREFIX PIP_* GIT_* BASH_ENV ENV CDPATH SHELLOPTS BASHOPTS GLOBIGNORE"
  for name in HOME GIT_CONFIG_GLOBAL GIT_CONFIG_NOSYSTEM LANG LC_ALL LC_CTYPE PATH TMPDIR PYTHON CC CXX SDKROOT EIGEN3_INCLUDE_DIR PYTHON_INCLUDE_DIR PYTHON_LIBRARY JOBS PYTEST_DISABLE_PLUGIN_AUTOLOAD; do
    if [[ -n ${!name:-} ]]; then
      printf '%s=%s\n' "$name" "${!name}"
    fi
  done
...
(
  ...
  export PATH="$PYTHON_BIN:$PREFIX/bin:$SAFE_PATH"
