...
class SaveBSE:
    ...
    def __init__(self, h5_file, bse_info, n_corr_shells, n_flavors, use_spin_orbit, nonlocal_order_parameter, beta,
        ...
        from chiq.index_pair import IndexPair, IndexPair2
...
def dcore_chiq(filename, np=1):
    ...
    #
    pars = create_parser(["model", "system", "impurity_solver", "mpi", "bse", "bse.h5_compression_options"])

    if "num_wb" not in pars.get_predefined_options("bse"):
        pars.add_option("bse", "num_wb", int, 1, "Number of bosonic frequencies (>0)")
        pars.add_option("bse", "num_wf", int, 10, "Number of fermionic frequencies (>0)")
        pars.add_option("bse", "h5_output_file", str, 'dmft_bse.h5', "Output HDF5 file for bse data")
        pars.add_option("bse", "skip_X0q_if_exists", bool, False, "[NOT USED] Skip X_0(q) calc if file already exists", OptionStatus.RETIRED)
        pars.add_option("bse", "skip_X0q", bool, False, "Skip X_0(q) calc")
        pars.add_option("bse", "skip_Xloc", bool, False, "Skip X_loc calc (for RPA)")
        pars.add_option("bse", "calc_only_chiloc", bool, False, "Calculate only chi_loc but no X_loc (for SCL, rRPA). Do not activate skip_Xloc when using this option.")
        pars.add_option("bse", "use_temp_file", bool, False, "Whether or not temporary file is used in computing X0_q. This option will reduce the memory footprints.")
        pars.add_option("bse", "X0q_qpoints_saved", str, 'quadrant', "Specifies for which q points X0q are saved in a HDF file. quadrant or path to a q_path.dat file.")

    pars.add_option("bse", "h5_compression", str, "gzip", "Compression algorithm for HDF5 output")
    pars.add_option("bse", "h5_compression_opts", int, 4, "Compression level of gzip for HDF5 output")

    # Options for other compression filters offered by hdf5plugin
    pars.allow_undefined_options("bse.h5_compression_options")
