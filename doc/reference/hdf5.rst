...
The columns from 'bse' to 'scl' indicate whether the data is referred to by ``chiq_main`` and ``chiq_post`` when the corresponding approximation scheme (selected by 'type' parameter) is specified. The last two columns indicate data access by ``calc_Iq`` and ``calc_Iq_scl``.
...
- ◆: Output
- ◇\*3: Used as input. ``chiq_main`` with mode='chi0' should be run in advance.
- ◇\*4: Used as input. ``chiq_main`` with mode='bse' should be run in advance.
