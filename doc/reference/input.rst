...
``bse.in`` is a file that contains parameters for ``chiq_main`` and ``chiq_post`` in the TOML format.
...
The ``[chiq_common]`` section contains general settings that apply to both programs ``chiq_main`` and ``chiq_post``. In contrast, the ``[chiq_main]`` and ``[chiq_post]`` sections provide configurations that are specific to each program.
...
.. csv-table::
   ...
   "omega_q", "string", "Not specified", "Filename storing the omega-q list to be computed. If not specified, omega-q list are generated from X0_q in the input HDF5 file."
...
``qpath.in`` is an input file for ``gen_qpath`` for generating a q-path.
...
``eigenvec.in`` is an input file for ``chiq_post`` to specify the order of the eigenvectors when ``mode = "eigen"`` and ``order = "file"``.
...
The format is the same as that of the eigenvectors files generated by ``chiq_main``, so see :ref:`the reference of the output files <output_eigenvec_dat>` for details.

``coeff.in``
------------

``coeff.in`` is an input file for ``chiq_post`` to specify the coefficients for the linear combination of the density operators when ``mode = "linear_combination"``.
