...
.. code-block:: bash
    ...
    chiq_fft [-h] [-f FILE_BSE] [--input_dname INPUT_DNAME]
...
``chiq_fft`` is a script for performing Fast Fourier Transform (FFT) from q-domain to r-domain. The input HDF5 file should contain full-Brillouin-zone data computed by ``chiq_main`` or ``calc_Iq_scl`` with q-points generated by ``gen_allq``. See Eq. :eq:`Ir` for the explicit equation.
