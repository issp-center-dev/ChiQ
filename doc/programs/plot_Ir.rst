...
.. code-block:: bash
    ...
    plot_Ir [-h] [--data_out DATA_OUT] [--format FORMAT] [--label LABEL] [--label-fontsize LABEL_FONTSIZE]
...
``plot_Ir`` is a script for plotting the interactions :math:`I_r` in the real space. The data can be generated by ``chiq_fft`` + ``chiq_post``.
