...
.. code-block:: bash
    ...
    plot_chiq_path [-h] [-d] [--mode {chi,chi0,rpa,scl,rrpa,Iq}]
...
``plot_chiq_path`` is a script for plotting the susceptibility along a q-path.

Positional Arguments
---------------------

**file_qpath**
    File name for the q-path data, e.g. ``"q_path.dat"``, generated by ``gen_qpath``.

**file_eigen**
    File name for eigenvalues of ``chi_q``, e.g. ``"chi_q_eigen.dat"``, generated by ``chiq_post`` with ``mode="eigen"``.
...
.. code-block:: console
    ...
    $ plot_chiq_path ../q_path.dat chi_q_eigen.dat --data_out chi_q_eigen_path.dat
    ...
