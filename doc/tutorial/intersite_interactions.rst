...
Before running ``dcore_chiq``, we need to prepare the ``q_fbz.dat`` file by using the ``gen_allq`` script.

.. code-block:: bash

  gen_allq dmft_square.in

Run ``dcore_chiq`` and ``chiq_main`` to calculate the susceptibility in the momentum space, :math:`\chi(q)` and :math:`\chi_\text{loc}(q)`.

.. code-block:: bash

  dcore_chiq --np 4 dmft_square.in
...
Once these susceptibilities are calculated, we can run ``calc_Iq`` to calculate the intersite interactions in the momentum space, :math:`I(q)`.

.. code-block:: bash

  calc_Iq -f dmft_bse.out.h5 --remove 1

``-f`` is used to specify the HDF5 file, which is the output file of ``chiq_main`` (in this case, ``dmft_bse.out.h5``).
...
Next, we perform the FFT from the momentum space :math:`I(q)` to the real space :math:`I(r)` by using ``chiq_fft``.

.. code-block:: bash

  chiq_fft -f dmft_bse.out.h5 --input_dname I_q --output_dname I_r dmft_square.in

``-f`` is used to specify the HDF5 file, which is used in ``calc_Iq`` (in this case, ``dmft_bse.out.h5``).
...
In this case, ``chiq_fft`` reads the :math:`I(q)` from ``dmft_bse.out.h5/bse/output/I_q`` and writes the :math:`I(r)` to ``dmft_bse.out.h5/bse/output/I_r``.

Finally, as in the case of :math:`\chi(q)`, we need to transform (or diagonalize) :math:`I(q)` to :math:`I(r)` by using ``chiq_post``.

.. code-block:: bash

  mpiexec --np 4 chiq_post bse.in
...
.. code-block:: bash
  ...
  plot_chiq_path --label ../label2.in --mode=Iq ../q_path.in I_q_eigen.dat
